...
The data is fetched with `load_ircolitis_cd8()`, a small build-time
helper (see `vignettes/ircolitis.R` in the package sources) that
downloads the 121 MB `.h5ad.gz` from GEO and parses it with `rhdf5`. It
is not part of presto – presto works on any counts matrix, `Seurat`, or
`SingleCellExperiment`; this just supplies a realistic example.
